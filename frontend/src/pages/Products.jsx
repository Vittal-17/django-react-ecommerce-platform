...
const Products = () => {
  ...
  // UI State: Fast, local state just for the slider visuals
  ...
  // Engine State: Debounced state for the heavy grid math
  const [debouncedMin, setDebouncedMin] = useState(0);
  const [debouncedMax, setDebouncedMax] = useState(1000);

  // NEW: The Debounce Engine (Eliminates the Flicker completely)
  useEffect(() => {
    const timer = setTimeout(() => {
      setDebouncedMin(minPrice);
      setDebouncedMax(maxPrice);
    }, 500); // Waits 150ms after you stop dragging before updating the grid
  ...
  // Unified Filtering Logic (Now powered by the debounced variables)
  useEffect(() => {
    let result = [...products];
    ...
    // Grid only updates based on the delayed Engine State
  ...
  const addToCart = async (product) => {
    ...
    if (user) {
      ...
      if (existingIndex >= 0) {
        if (tempCart[existingIndex].quantity + qty > product.stock) { toast.error(`Cannot exceed stock limit`); return; }
        ...
        <AnimatePresence>
          {showFilters && (
            <AdvancedFilterPanel
              ...
              animate={{ opacity: 1, height: 'auto', padding: '1.5rem 2rem' }}
              exit={{ opacity: 0, height: 0, padding: 0 }}
            >
              
              {/* LEFT COLUMN: Anchored to the right side of its cell */}
                ...
                <PriceSliderContainer>
                  ...
                  <SliderTrack>
                    ...
                    <ThumbInput
                      ...
                      style={{ zIndex: minPrice > highestPrice - (highestPrice * 0.1) ? 5 : 3 }}
                    />
              ...
              {/* RIGHT COLUMN: Anchored to the left, wraps the button to hold exact space */}
            ...
            return (
              <ProductCard key={product.id} initial={{ opacity: 0, y: 15 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: Math.min(index * 0.05, 0.3) }} whileHover={{ y: -4 }}>
                ...
                <ProductInfo>
                  ...
                  {isAtLimit ? (
                    <LimitWarning>Only {product.stock} left in stock!</LimitWarning>
                  ) : product.stock === 0 ? (
                    <LimitWarning>Out of Stock</LimitWarning>
                  ) : null}
...
const AdvancedFilterPanel = styled(motion.div)` 
  display: grid; 
  /* The Magic Formula: Left cell & Right cell match perfectly (1fr), Middle is auto */
  ...
  @media (max-width: 900px) { 
    grid-template-columns: 1fr; /* Stacks nicely on mobile */
...
const ClearButtonWrapper = styled.div`
  width: 120px; /* Guarantees the layout never collapses when button unmounts */
  display: flex;
  align-items: center;
`;
...
const ThumbInput = styled.input`
  ...
  &::-webkit-slider-thumb { -webkit-appearance: none; pointer-events: auto; width: 20px; height: 20px; background: white; border: 3px solid #4CAF50; border-radius: 50%; cursor: pointer; box-shadow: 0 2px 6px rgba(0,0,0,0.15); }
  &::-moz-range-thumb { pointer-events: auto; width: 20px; height: 20px; background: white; border: 3px solid #4CAF50; border-radius: 50%; cursor: pointer; box-shadow: 0 2px 6px rgba(0,0,0,0.15); }
`;
...
const ClearButton = styled(motion.button)` 
  ...
  background: #ffebee; color: #d32f2f; border: none; border-radius: 8px; 
