...
// The new interactive trigger button
const StatusTriggerButton = styled(motion.button)`
  display: flex;
  align-items: center;
  gap: 0.5rem;
  padding: 0.5rem 1rem;
  ...
  &:disabled {
    cursor: not-allowed;
    opacity: 0.7;
    &:hover { filter: none; box-shadow: none; }
  }
`;
...
const ProductInfo = styled.div`
  ...
  .details { font-size: 0.9rem; color: #666; }
`;
...
const OrdersSection = () => {
  ...
  const handleStatusChange = async (orderId, newStatus) => {
    if (newStatus === 'cancelled') {
      if (!window.confirm('Cancel this order? This will restore the items to the inventory.')) {
        return;
      }
    }
  ...
  const getStatusConfig = (status) => {
    switch (status) {
      case 'pending': 
        return { bg: '#fef9c3', fg: '#713f12', border: '#eab308', icon: <FaClock />, label: 'Pending' };
      case 'shipped': 
        return { bg: '#dbeafe', fg: '#1e40af', border: '#3b82f6', icon: <FaShippingFast />, label: 'Shipped' };
      case 'delivered': 
        return { bg: '#dcfce7', fg: '#166534', border: '#22c55e', icon: <FaCheckCircle />, label: 'Delivered' };
      case 'cancelled': 
        return { bg: '#fee2e2', fg: '#991b1b', border: '#ef4444', icon: <FaTimesCircle />, label: 'Cancelled' };
      default: 
        return { bg: '#f3f4f6', fg: '#374151', border: '#9ca3af', icon: <FaClock />, label: 'Unknown' };
    }
  };
    ...
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }}>
      ...
      <ul style={{ padding: 0, listStyle: 'none' }}>
        {orders.map(o => {
          ...
          return (
            <ListItem key={o.id} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.3 }}>
              <OrderDetails>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', flexWrap: 'wrap', gap: '1rem' }}>
                  <div>
                    ...
                    <div style={{ color: '#2e7d32', fontWeight: 'bold', marginTop: '0.3rem', fontSize: '1.1rem' }}>
                      Total: ${Number(o.total_price).toFixed(2)}
                    </div>
                  </div>
                  
                  {/* The Trigger Button replaces the Dropdown */}
                  <StatusTriggerButton 
                    $bg={config.bg} 
                    $fg={config.fg}
                    ...
                    title={isLocked ? "Status is locked" : "Click to update status"}
                  >
                        ...
                        <ProductRow key={index}>
                          <ProductImage>
                            {productInfo?.image_url ? (
                              ...
                              <span style={{ fontSize: '0.7rem', color: '#aaa' }}>No Img</span>
                            )}
                          </ProductImage>
                          <ProductInfo>
                            <span className="name">{productInfo?.name || item.name || `Product ID #${item.product}`}</span>
                            <span className="details">Qty: {item.quantity} &nbsp;|&nbsp; ${Number(item.price).toFixed(2)} each</span>
                          </ProductInfo>
                        </ProductRow>
                      );
                    })
                  ) : (
                    <div style={{ color: '#757575', fontStyle: 'italic' }}>No item details available.</div>
                  )}
                </ItemsContainer>
              </OrderDetails>
            </ListItem>
                ...
                {['pending', 'shipped', 'delivered', 'cancelled'].map(statusType => {
                  ...
                  return (
                    <StatusOptionBtn
                      ...
                      $bg={optConfig.bg}
                      $fg={optConfig.fg}
                      $borderColor={optConfig.border}
                      $active={isActive}
                      onClick={() => {
                        if (!isActive) handleStatusChange(activeOrder.id, statusType);
                      }}
                      whileHover={!isActive ? { scale: 1.02 } : {}}
                      whileTap={!isActive ? { scale: 0.98 } : {}}
                    >
                      {optConfig.icon}
                      <span style={{ flex: 1, textAlign: 'left' }}>{optConfig.label}</span>
                      {isActive && <span style={{ fontSize: '0.8rem', opacity: 0.7 }}>(Current)</span>}
                    </StatusOptionBtn>
                  );
                })}
              </StatusOptionList>
