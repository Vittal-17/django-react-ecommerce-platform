...
const Navbar = () => {
  ...
  const handleLogout = () => {
    setIsLoggingOut(true); // 🚀 Lock down the screen instantly

    setTimeout(() => {
      logoutUser(); // Clears user context in the background
      setIsLoggingOut(false); // 🚀 Drop the spinner
      
      // Wait 250ms for the exit animation to clear the screen, then show the toast
      setTimeout(() => {
        toast.success('👋 Logged out securely!', { duration: 3000, id: 'logout-success' });
      }, 250);
      
    }, 1000); // 1-second artificial delay for the animation
  };

  // 🚀 Converted to a function to prevent unnecessary React re-mounts on scroll
  const renderNavLinks = (isMobile) => (
    <>
      ...
      {(user?.role === 'seller') && (
        <CustomLink to="/vendor" label="Vendor HQ" icon={<FaTruck />} onClick={isMobile ? closeMobileMenu : undefined} />
      )}
  ...
  return (
    <>
      {/* 🚀 The Full-Screen Blocker is now safely in the DOM! */}
      <AnimatePresence>
